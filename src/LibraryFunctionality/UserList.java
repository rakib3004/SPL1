...
public class UserList {
    ...
    public void addUser(String textUserID, String[] infoDataArray) throws IOException {
         ...
         * //user id------user name------user institute ------- higher/secondary----
         * class no/dept name
         * fileWriter.write(textUserID+"\t"+infoDataArray[0]+"\t"+infoDataArray[1]+"\t"+
         * infoDataArray[3]+"\t"+infoDataArray[2]+"\n");
         * fileWriter.close();
         */
        ...
        try {
            out = new BufferedWriter(
                    new FileWriter("src" + File.separator + "LibraryFunctionality" + File.separator + "UserTable.txt",
                            true));
        } catch (IOException e) {
            e.printStackTrace();
        }
        try {
            out.write(textUserID + "\t" + infoDataArray[0] + "\t" + infoDataArray[1] + "\t" + infoDataArray[3] + "\t"
                    + infoDataArray[2] + "\n");
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public LibraryUser[] parseUser() throws IOException {
        ...
        for (dataIterator = 0; userInfoData[dataIterator] != '\0'; dataIterator++) {
            ...
            if (userInfoData[dataIterator] == '\t') {
                ...
                libraryUser[userIterator] = new LibraryUser(userID, userName, userInstitute, userEducationLevel,
                        userClass);
                System.out.println(
                        userID + "\t" + userName + "\t" + userInstitute + "\t" + userEducationLevel + "\t" + userClass);
                dataArray[0] = "";
                dataArray[1] = "";
                dataArray[2] = "";
                dataArray[3] = "";
